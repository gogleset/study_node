...
(async () => {
  ...
  try {
    const sql =
      "SELECT s.studno, d.dname, s.grade, s.name FROM student s, department d WHERE s.deptno = d.deptno AND d.deptno IN (SELECT deptno FROM department WHERE dname LIKE ?)";
    const input_data = ["%공학%"];
    const [result1] = await dbcon.query(sql, input_data);

    result1.map((v, i) => {
      console.log("%s, %s, %s, %s", v.studno, v.dname, v.grade, v.name);
    });
  } catch (err) {
    console.log(err);
