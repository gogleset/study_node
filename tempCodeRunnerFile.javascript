...
(async () => {
    ...
    try {
        dbcon = await mysql2.createConnection({
            host: 'localhost', // MYSQL 서버 주소 (다른 PC인 경우 IP 주소)
            port: 3306, // MYSQL 설치 시, 기본 값 3306
            user: 'root', // 접근 권한 아이디 (root = 관리자)
            password: 'Chlwls@213468', // 설치 시 입력한 비밀번호
    ...
    try {
        ...
        // // UPDATE, DELETE 쿼리의 경우 사용할수 없는 값임
        ...
        // UPDATE, DELETE 쿼리의 경우 사용할수 없는 값임
        console.log('수정된 데이터 수: ' + result2.affectedRows);

        const [result3] = await dbcon.query('DELETE FROM department WHERE deptno > ?', [300]);
        // UPDATE, DELETE 쿼리의 경우 사용할수 없는 값임
        console.log('삭제된 데이터 수: ' + result3.affectedRows);
    } catch (err) {
        console.log(err);
        // 앞 처리 과정에서 db에 접속된 상태이므로, SQL 실행 도중 문제가 발생하면 DB 접근을 해제해야 한다
        dbcon.end();
        return;
    }
    dbcon.end();
